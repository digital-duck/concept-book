...
function _localizePath(file, level, lang, model = '') {
  // New format: output/{level}.{lang}/{model}/html/  (4-segment)
  if (model && /output\/[^/]+\/[^/]+\/html\//.test(file)) {
    return file.replace(/output\/[^/]+\/[^/]+\/html\//, `output/${level}.${lang}/${model}/html/`)
  }
  // Legacy format: output/{level}.{lang}/html/  (3-segment)
...
function _injectConceptBooksSection(win, doc, domainId, books, genConcepts, level, lang) {
  ...
  const _WARN_STYLE = 'margin-top:4px;font-size:11px;color:#dc2626;display:none'
    ...
    <div style="${_ROW}">
      <select id="cb-book-sel" style="${_SEL}">
        ...
        ${sortedBooks.map(b => {
          ...
          return `<option value="${_localizePath(b.file, level, lang, b.model || '')}">${label}</option>`
        }).join('')}
      </select>
    ...
    <div style="${_ROW}">
      <select id="cb-cpt-sel" style="${_SEL}">
        ...
        ${sortedConcepts.map(c => {
          ...
          return `<option value="${_localizePath(c.file, level, lang, c.model || '')}">${label}</option>`
        }).join('')}
      </select>
  ...
  // Checks if a concept-book HTML file actually exists by looking for the
  // spl-credit marker. Vite dev server returns 200+index.html for all missing
  // static files, so HTTP status alone can't be used.
  ...
  if (sortedBooks.length > 0) {
    ...
    btn.addEventListener('click', async () => {
      if (!sel.value) return
      const url = `${import.meta.env.BASE_URL}domains/${domainId}/${sel.value}`
      if (await _fileExists(url)) {
        win.open(url, '_blank')
      } else {
        _showWarn(warn, `No content available for language "${lang}". Generate it first.`)
      }
    })
  }
...
function _injectGenerateSection(win, doc, domainId, capstone, level, lang, books = []) {
  ...
  btn.addEventListener('click', () => {
    ...
    const url = `/api/generate?domain=${encodeURIComponent(domainId)}&target=${encodeURIComponent(target)}&level=${encodeURIComponent(level)}&language=${encodeURIComponent(lang)}&model=${encodeURIComponent(model)}${skipCache ? '&skip_cache=true' : ''}`
